...
print("Found:", c)
r = [a for a in
     SeqIO.parse(Entrez.efetch(db="nucleotide", rettype="gb", retmode="text", retmax=c, webenv=w, query_key=q), "gb") if
     l <= len(a.seq) <= H]
print("Filtered:", len(r))
d = p.DataFrame([{"Acc": a.id, "Len": len(a.seq), "Desc": a.description} for a in r])
f = f"taxid_{t}"
d.to_csv(f + ".csv", index=0)
d = d.sort_values("Len", 0)
m.plot(d["Acc"], d["Len"], 'o-');
m.xticks(rotation=90, fontsize=6)
m.tight_layout();
m.savefig(f + ".png")
